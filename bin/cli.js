#!/usr/bin/env node
// steam-wrap — wrap a built web game (dist/) in Electron + Steamworks.
//
//   steam-wrap dev            run the game in Electron with live Steam
//   steam-wrap build <os>     depot-ready folder in dist-electron/<os>  (mac|win|linux)
//
// Configuration is read from the game's package.json "steamWrap" block —
...
const PKG_ROOT = path.join(__dirname, '..');      // the steam-wrap package
...
function loadConfig() {
    ...
    const extendPath = path.join(PROJECT, c.extend ?? 'steam-wrap.extend.cjs');
...
function webBuild(cfg) {
    if (cfg.hasWebBuild) {
        console.log('steam-wrap: running npm run build');
        execSync('npm run build', { cwd: PROJECT, stdio: 'inherit' });
    }
    if (!fs.existsSync(path.join(cfg.distDir, 'index.html'))) {
        fail(`no index.html in ${cfg.distDir} — build your game first or set steamWrap.dist`);
    }
}
...
function dev(cfg) {
    ...
    const res = spawnSync(electron, [path.join(PKG_ROOT, 'runtime', 'main.cjs')], {
        cwd: PROJECT,
        stdio: 'inherit',
        env: { ...process.env, STEAM_WRAP_CONFIG: JSON.stringify(cfg) },
    });
...
async function build(cfg, platform) {
    ...
    const stage = path.join(PROJECT, '.steam-wrap');
    ...
    console.log(`steam-wrap: depot-ready build in dist-electron/${platform}/`);
}
...
if (cmd === 'dev') {
    ...
    console.log(`steam-wrap — wrap a web game in Electron + Steamworks

Usage:
  steam-wrap dev            run the game in Electron (Steam works if the client is running)
  steam-wrap build <os>     depot-ready build in dist-electron/<os>   (mac | win | linux)
...
Config (all optional) in your package.json:
  "steamWrap": {
    ...
    "extend": "steam-wrap.extend.cjs" // optional main-process hook
  }`);
